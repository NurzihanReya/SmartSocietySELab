...
if (isset($_POST['submit'])) {
  ...
  if ($password == $confirm_password) {
    $sql = "SELECT * FROM users WHERE name='$full_name'";
		$result = mysqli_query($conn, $sql);
    $sql2 = "SELECT * FROM organizations WHERE name='$full_name'";
		$result2 = mysqli_query($conn, $sql2);
		if ($user_name=="user") {
			if(!$result->num_rows > 0){
        $sql = "INSERT INTO users (name, email, dob, phone, street, house, password)
            VALUES ('$full_name', '$email', '$dob', '$phone', '$street', '$house', '$password')";
        $result = mysqli_query($conn, $sql);
        if ($result) {
      ...
      $sql = "INSERT INTO organizations (name, email, phone, street, house, b_id, password)
          VALUES ('$full_name', '$email', '$phone', '$street', '$house', '$block', '$password')";
      $result = mysqli_query($conn, $sql);
      if ($result) {
